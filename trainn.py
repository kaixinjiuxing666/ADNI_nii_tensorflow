...
def main(task='all'):
    ...
    for epoch in range(0, n_epoch+1):
        ...
        for batch in tl.iterate.minibatches(inputs=X_train, targets=y_train,
            ...
            data = tl.prepro.threading_data([_ for _ in zip(images[:,:,:,:, np.newaxis], labels)],
                                             fn=distort_imgs)
            ...
            b_images = b_images.transpose((0,2,3,1,4,5))#(10,240,240,4,1,1)
        ...
        for batch in tl.iterate.minibatches(inputs=X_test, targets=y_test,
            ...
            print(b_labels.shape)#(2,256,256,1,1)
            b_labels.shape = (batch_size, nw, nh, nz)
